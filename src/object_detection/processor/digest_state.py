...
class DigestStateManager:
    ...
    def _save_state(self) -> None:
        ...
        data = {
            ...
            "digests": {
                digest_id: state.to_dict() for digest_id, state in self.states.items()
            },
        }
    ...
    def initialize_digest(
        ...
        if digest_id in self.states:
            ...
            if digest_id not in config_digest_ids:
                logger.info(
                    f"Digest '{digest_id}' removed from config, discarding state"
                )
        ...
        if digest_id in config_digest_ids:
            ...
            self._save_state()
            logger.info(
                f"Created new period for '{digest_id}', ends {state.period_end}"
            )
            return state
    ...
    def _maybe_checkpoint(self) -> None:
        ...
        should_checkpoint = (
            self.events_since_checkpoint >= CHECKPOINT_INTERVAL_EVENTS
            or time.time() - self.last_checkpoint_time > CHECKPOINT_INTERVAL_SECONDS
        )

        if should_checkpoint:
            self._save_state()
            logger.debug(
                f"Checkpointed digest state ({self.events_since_checkpoint} events)"
            )
    ...
    def start_new_period(
        self, digest_id: str, period_minutes: int
    ) -> DigestPeriodState:
