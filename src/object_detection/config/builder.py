...
class ConfigBuilder:
    ...
    def _start_preview_server(self):
        ...
        try:
            ...
            print(f"{Colors.GRAY}Frames will still be saved to {self.preview_dir}/{Colors.RESET}")
    ...
    def _setup_camera(self) -> bool:
        ...
        print(f"{Colors.GREEN}OK{Colors.RESET} ({self.frame_width}x{self.frame_height})")
    ...
    def _setup_detection(self):
        ...
        default_conf = "0.3"
        conf_str = input(f"Confidence threshold [{default_conf}]: ").strip() or default_conf
        conf = float(conf_str)
    ...
    def _setup_lines(self):
        ...
        while True:
            ...
            # Adjust option
            while True:
                action = input("  [c] Capture again  [a] Adjust position  [n] Next: ").strip().lower()
                if action == 'c':
                    self._capture_annotated_preview(lines=lines)
                    print(f"  {Colors.GREEN}Preview updated{Colors.RESET}")
                elif action == 'a':
                    if line_type == 'vertical':
                        pos_str = input(f"  New position % from left [{position}]: ").strip() or str(position)
                    else:
                        pos_str = input(f"  New position % from top [{position}]: ").strip() or str(position)
                    position = int(pos_str)
    ...
    def _setup_zones(self):
        ...
        while True:
            ...
            # Adjust option
            while True:
                action = input("  [c] Capture again  [a] Adjust bounds  [n] Next: ").strip().lower()
                if action == 'c':
                    self._capture_annotated_preview(lines=lines, zones=zones)
                    print(f"  {Colors.GREEN}Preview updated{Colors.RESET}")
                elif action == 'a':
    ...
    def _setup_events(self):
        ...
        print(f"{Colors.GRAY}Events define what to detect and what actions to take{Colors.RESET}")
        ...
        if not lines and not zones:
            print(f"{Colors.YELLOW}No lines or zones defined - skipping events{Colors.RESET}")
            return
        ...
        while True:
            ...
            if pdf != 'n':
                report_id = input("    Report ID [traffic_report]: ").strip() or "traffic_report"
    ...
    def _setup_pdf_reports(self):
        ...
        for report_id in report_ids:
            ...
            report_events = [e['name'] for e in events if e.get('actions', {}).get('pdf_report') == report_id]

            title = input(f"    Title [{report_id.replace('_', ' ').title()}]: ").strip()
            if not title:
            ...
            annotate = False
            if has_photos:
                annotate_str = input("    Annotate photos with lines/zones? (Y/n): ").strip().lower()
    ...
    def _setup_output(self):
        ...
        if has_frame_capture:
            ...
            self.config['frame_storage'] = {
                'type': 'local',
                'local_dir': frames_dir
            }
    ...
    def _capture_annotated_preview(self, lines: List[dict] = None, zones: List[dict] = None):
        ...
        if lines:
            for line in lines:
                ...
                if line_type == 'vertical':
                    ...
                    cv2.putText(frame, description, (x + 5, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    # Show percentage
                    cv2.putText(frame, f"{position_pct}%", (x + 5, 55),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                else:
                    y = int(height * position_pct / 100)
                    cv2.line(frame, (0, y), (width, y), (0, 255, 255), 2)
                    cv2.putText(frame, description, (10, y - 10),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    cv2.putText(frame, f"{position_pct}%", (10, y + 20),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        ...
        if zones:
            for zone in zones:
                ...
                cv2.putText(frame, description, (x1 + 5, y1 + 25),
