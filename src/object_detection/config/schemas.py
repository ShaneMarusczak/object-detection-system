...
class LineConfig(BaseModel):
    ...
    allowed_classes: Optional[List[int]] = Field(default=None, description="COCO class IDs allowed to cross")
...
class EventMatch(BaseModel):
    ...
    event_type: Optional[Literal["LINE_CROSS", "ZONE_ENTER", "ZONE_EXIT", "ZONE_DWELL"]] = None
...
class Config(BaseModel):
    ...
    speed_calculation: SpeedCalculationConfig = Field(default_factory=SpeedCalculationConfig)
    ...
    def validate_references(self):
        ...
        for event in self.events:
            if event.match.line and event.match.line not in line_names:
                raise ValueError(f"Event '{event.name}' references non-existent line: '{event.match.line}'")
