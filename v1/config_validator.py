...
def validate_config(config):
    ...
    # If any errors found, raise with all messages
    if errors:
...
def _validate_detection(config):
    """Validate detection settings"""
    errors = []
    ...
    if "horizontal" in roi:
        ...
        if h_roi.get("enabled", False):
            if "crop_from_left_pct" not in h_roi:
                errors.append(
                    "roi.horizontal.crop_from_left_pct is required when enabled"
                )
            else:
            ...
            if "crop_to_right_pct" not in h_roi:
                errors.append(
                    "roi.horizontal.crop_to_right_pct is required when enabled"
                )
            else:
    ...
    if "vertical" in roi:
        ...
        if v_roi.get("enabled", False):
            ...
            if "crop_to_bottom_pct" not in v_roi:
                errors.append(
                    "roi.vertical.crop_to_bottom_pct is required when enabled"
                )
            else:
...
def _validate_lines(config):
    ...
    for i, line in enumerate(lines):
        ...
        if "description" not in line:
            ...
            not isinstance(line["description"], str) or not line["description"].strip()
        ):
        ...
        if "allowed_classes" in line:
            ...
            if not isinstance(allowed, list):
                ...
                errors.append(
                    f"{line_id}.allowed_classes contains classes not in detection.track_classes: {missing}"
                )

    return errors


def _validate_zones(config):
    ...
    for i, zone in enumerate(zones):
        ...
        if "description" not in zone:
            ...
            not isinstance(zone["description"], str) or not zone["description"].strip()
        ):
        ...
        if "allowed_classes" in zone:
            ...
            if not isinstance(allowed, list):
                ...
                missing = set(allowed) - track_classes
                errors.append(
                    f"{zone_id}.allowed_classes contains classes not in detection.track_classes: {missing}"
                )
...
def _validate_camera(config):
    """Validate camera settings"""
    errors = []

    if "camera" not in config:
        errors.append("Missing 'camera' section")
        return errors
...
def print_validation_summary(config):
    ...
    if h_roi.get("enabled") or v_roi.get("enabled"):
        ...
        if h_roi.get("enabled"):
            print(
                f"  Horizontal: {h_roi.get('crop_from_left_pct')}% - {h_roi.get('crop_to_right_pct')}% from left"
            )
        if v_roi.get("enabled"):
            print(
                f"  Vertical: {v_roi.get('crop_from_top_pct')}% - {v_roi.get('crop_to_bottom_pct')}% from top"
            )
    ...
    if lines:
        ...
        for i, line in enumerate(lines, 1):
            ...
            print(
                f"  {line['type'][0].upper()}{i}: {line['position_pct']}% - \"{line['description']}\" (classes: {allowed})"
            )
    ...
    if zones:
        ...
        for i, zone in enumerate(zones, 1):
            ...
            print(
                f"  Z{i}: ({zone['x1_pct']},{zone['y1_pct']})-({zone['x2_pct']},{zone['y2_pct']})% - \"{zone['description']}\" (classes: {allowed})"
            )
