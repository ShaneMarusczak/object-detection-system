...
def print_validation_summary(config):
    ...
    if h_roi.get('enabled') or v_roi.get('enabled'):
        ...
        if h_roi.get('enabled'):
            print(f"  Horizontal: {h_roi.get('crop_from_left_pct')}% - {h_roi.get('crop_to_right_pct')}% from left")
        if v_roi.get('enabled'):
            print(f"  Vertical: {v_roi.get('crop_from_top_pct')}% - {v_roi.get('crop_to_bottom_pct')}% from top")
    ...
    if lines:
        ...
        for i, line in enumerate(lines, 1):
            ...
            print(f"  {line['type'][0].upper()}{i}: {line['position_pct']}% - \"{line['description']}\" (classes: {allowed})")
    ...
    if zones:
        ...
        for i, zone in enumerate(zones, 1):
            ...
            print(f"  Z{i}: ({zone['x1_pct']},{zone['y1_pct']})-({zone['x2_pct']},{zone['y2_pct']})% - \"{zone['description']}\" (classes: {allowed})")
