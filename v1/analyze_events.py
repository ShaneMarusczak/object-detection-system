...
def analyze_events(data_queue, config, model_names):
    ...
    try:
        ...
        if event_count > 0:
            ...
            print("  No events recorded - file will be empty")


def _enrich_event(
    event, model_names, line_descriptions, zone_descriptions, start_time, speed_enabled
):
    """
    ...
    iso_timestamp = datetime.fromtimestamp(
        absolute_timestamp, tz=timezone.utc
    ).isoformat()
...
def _print_event(event, level, event_count):
    ...
    if event_type == "LINE_CROSS":
        ...
        if "speed_px_per_sec" in event:
            ...
            print(
                f"#{event_count:4d} | Track {track_id:3d} ({obj_name:12s}) crossed {line_id} ({line_desc:20s}) {direction:3s} @ {speed:6.1f} px/s"
            )
        else:
            print(
                f"#{event_count:4d} | Track {track_id:3d} ({obj_name:12s}) crossed {line_id} ({line_desc:20s}) {direction:3s}"
            )
        ...
        print(
            f"#{event_count:4d} | Track {track_id:3d} ({obj_name:12s}) entered {zone_id} ({zone_desc})"
        )
        ...
        print(
            f"#{event_count:4d} | Track {track_id:3d} ({obj_name:12s}) exited  {zone_id} ({zone_desc}) - {dwell:.1f}s dwell"
        )
...
if __name__ == "__main__":
    ...
    model_names = {0: "person", 15: "cat", 16: "dog"}
