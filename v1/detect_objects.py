...
def run_detection(data_queue, config):
    ...
    if roi_config['enabled']:
        print(f"  ROI: Enabled (H: {roi_config['h_from']}-{roi_config['h_to']}%, "
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for track_id, box, obj_class in zip(track_ids, xyxy, classes):
                        ...
                        for line in lines:
                            ...
                            if crossed:
                                ...
                                if speed_enabled and track_id in first_seen_positions:
                                    ...
                                    distance = abs(center_x - first_x) if line['type'] == 'vertical' else abs(center_y - first_y)
                    ...
                    for zone in zones:
                        ...
                        inside = (zone_x1 <= center_x <= zone_x2 and 
...
def _parse_lines(config):
    ...
    for line_config in config.get('lines', []):
        ...
        allowed_classes = line_config.get('allowed_classes', 
